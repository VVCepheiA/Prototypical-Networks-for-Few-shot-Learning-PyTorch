...
os.environ['R_HOME'] = '/usr/local/Cellar/r/3.5.1/lib/R'


def prepare_GO_data(adata, gene2go, GO_file, GO_min_genes=500, GO_max_genes=None, GO_min_level=3):
    """
    Preprocesses data .
...
def get_processed_tabula_muris_GO_data(adata, GO_min_genes=500, GO_max_genes=None, GO_min_level=3,
                                       data_dir='../../Maria/rna-seq/'):
    """
    ...
    mgi2go = map_mgi2go(data_dir + "data/gene_ontology/gene_association.mgi")
    ...
    GOobo_file = data_dir + "data/gene_ontology/go-basic.obo"

    processed_data = prepare_GO_data(adata, mgi2go, GO_file=GOobo_file,
                                     GO_min_genes=GO_min_genes, GO_max_genes=GO_max_genes, GO_min_level=GO_min_level)
